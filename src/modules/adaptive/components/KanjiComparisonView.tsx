  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
      <div className="relative w-full max-w-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Content Body */}
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 text-slate-900 dark:text-slate-100">
          {/* Side-by-side Grid */}
          <div className="grid grid-cols-2 gap-4 sm:gap-6">
            {/* Kanji A */}
            <div className="flex flex-col items-center gap-4 p-5 bg-slate-50 dark:bg-slate-950/60 rounded-2xl border border-blue-500/20">
              <span className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider">Target Kanji</span>
              {dataA ? (
                <>
                  <StrokeViewer kanji={dataA.literal} unicode={dataA.unicode} strokeCount={dataA.strokeCount} />
                  <div className="text-center flex flex-col gap-1">
                    <h2 className="text-4xl font-black text-slate-900 dark:text-white font-japanese">{dataA.literal}</h2>
                ...
                <span className="text-3xl font-bold text-slate-900 dark:text-white font-japanese">{kanjiA}</span>
              )}
            </div>

            {/* Kanji B */}
            <div className="flex flex-col items-center gap-4 p-5 bg-slate-50 dark:bg-slate-950/60 rounded-2xl border border-purple-500/20">
              <span className="text-xs font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider">Confusing Kanji</span>
              {dataB ? (
                <>
                  <StrokeViewer kanji={dataB.literal} unicode={dataB.unicode} strokeCount={dataB.strokeCount} />
                  <div className="text-center flex flex-col gap-1">
                    <h2 className="text-4xl font-black text-slate-900 dark:text-white font-japanese">{dataB.literal}</h2>
                    <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 capitalize">
                ...
                <span className="text-3xl font-bold text-slate-900 dark:text-white font-japanese">{kanjiB}</span>
              )}
            </div>
          </div>
          ...
          {reason && (
            <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl text-amber-800 dark:text-amber-200 text-sm flex items-start gap-3">
              <HelpCircle className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
              <div className="flex flex-col gap-1">
                <span className="font-bold text-amber-600 dark:text-amber-400 text-xs uppercase tracking-wider">Key Difference</span>
                <p className="font-medium text-slate-800 dark:text-slate-200">{cleanJMdictString(reason)}</p>
              </div>
            </div>
          )}
        </div>

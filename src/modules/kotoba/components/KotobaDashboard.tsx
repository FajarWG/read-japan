...
export function KotobaDashboard() {
  ...
  // ── States ─────────────────────────────────────────────────────────────────
  const [terms, setTerms] = useState<KotobaTerm[]>(DEFAULT_KOTOBA_TERMS);
  ...
  const text = {
    en: {
      title: "Kotoba (言葉)",
      subtitle: "Your personal notebook of Japanese words to memorize. Add anything new you come across.",
      backBtn: "Back",
      ...
      noTerms: "No words found matching your search. Add one with “Add Word”.",
  ...
  useEffect(() => {
    ...
    const savedCustom = localStorage.getItem("kotoba_custom");
    let customList: KotobaTerm[] = [];
    if (savedCustom) {
      try {
    ...
    const savedMastery = localStorage.getItem("kotoba_mastery");
    if (savedMastery) {
      try {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setTerms([...DEFAULT_KOTOBA_TERMS, ...customList]);
  }, []);
  ...
  const handleRating = (termId: string, rating: "again" | "hard" | "good" | "easy") => {
    ...
    localStorage.setItem("kotoba_mastery", JSON.stringify(updatedMastery));
  ...
  const handleDeleteCustomTerm = (e: React.MouseEvent, termId: string) => {
    ...
    if (savedCustom) {
      try {
        const customList: KotobaTerm[] = JSON.parse(savedCustom);
        const filtered = customList.filter((t) => t.id !== termId);
        localStorage.setItem("kotoba_custom", JSON.stringify(filtered));

        // Update active terms state
        setTerms([...DEFAULT_KOTOBA_TERMS, ...filtered]);
  ...
  const handleAddTerm = (e: React.FormEvent) => {
    ...
    localStorage.setItem("kotoba_custom", JSON.stringify(updatedCustom));
    setTerms([...DEFAULT_KOTOBA_TERMS, ...updatedCustom]);
  ...
  const handleCopyPrompt = () => {
    const promptText = `Tolong buatkan detail kosakata Jepang untuk kata: "${aiKeyword || "[kata yang Anda inginkan]"}" dalam format JSON. Ikuti struktur di bawah ini:
{
...
{`Tolong buatkan detail kosakata Jepang untuk kata: "${aiKeyword || "[kata yang Anda inginkan]"}" dalam format JSON. Ikuti struktur di bawah ini:
{
  "kanji": "[Tuliskan Kanji/Katakana untuk kata tersebut]",
  "hiragana": "[Tuliskan Furigana/Cara baca kata dalam Hiragana saja]",
  "translationId": "[Tuliskan arti kata dalam Bahasa Indonesia]",

...
export const JourneyTimeline: React.FC<JourneyTimelineProps> = ({ timeline }) => {
  return (
    <div className="flex flex-col gap-4 p-6 bg-slate-50 dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl text-slate-900 dark:text-slate-100">
      ...
      {timeline.length === 0 ? (
        <div className="p-4 text-center text-xs text-slate-500 bg-white dark:bg-slate-950/40 rounded-2xl border border-slate-200 dark:border-slate-800/60">
          No recent activity logged yet today. Start a review session!
        </div>
      ) : (
        <div className="flex flex-col gap-3 relative before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-800">
          {timeline.map((item) => (
            <div key={item.id} className="flex items-start gap-3 relative z-10">
              <div className="w-7 h-7 rounded-full bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 flex items-center justify-center text-blue-500 dark:text-blue-400 shrink-0">
